...
function monitorRAM() {
    const ramUsage = checkRAMUsage();
    console.log(`Current RAM usage: ${ramUsage.toFixed(2)}%`);

    if (remainingTime === undefined) {
        remainingTime = duration * 1000; // setting initial remainingTime
    }

    // Update the remaining time
    remainingTime -= checkInterval; // decrement the remaining time
    if (ramUsage > threshold || remainingTime <= 0) {
        console.log('RAM usage exceeds threshold or duration is over. Restarting script...');

        // Kill all existing workers and reset remaining time
        for (const id in cluster.workers) {
            cluster.workers[id].kill();
        }
        remainingTime = duration * 1000; // reset remainingTime
        initiateNewCluster(); // function to fork new workers
    }
}

function initiateNewCluster() {
    cluster.setupMaster({
        exec: 'https.js', // Adjust path as necessary
...
// Start monitoring
setInterval(monitorRAM, checkInterval);
...
const lang_header = [
  ...
  "da-DK,da;q=0.9,en;q=0.7",
  "ro-RO,ro;q=0.9,en;q=0.7",
  "ar-AR,ar;q=0.9,en;q=0.7",
  "vi-VN,vi;q=0.9,en;q=0.7",
  "th-TH,th;q=0.9,en;q=0.7",
  "ms-MY,ms;q=0.9,en;q=0.7",
